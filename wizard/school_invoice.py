...
class school_invoice(osv.osv_memory):
    """school Invoice"""

    _name = "school.invoice"
    _description = "school Invoice"
    _columns = {
        'product_id': fields.many2one('product.product','school', required=True),
    ...
    def school_invoice(self, cr, uid, ids, context=None):
        ...
        if data:
            data = data[0]
            datas = {
                'school_product_id': data.product_id.id,
                'amount': data.member_price
            }
        invoice_list = partner_obj.create_school_invoice(cr, uid, context.get('active_ids', []), datas=datas, context=context)
